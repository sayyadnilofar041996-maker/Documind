...
import { Mail, Lock, Loader2, BrainCircuit, ArrowRight, ShieldCheck } from 'lucide-react'
...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (success) {
      toast.success('Access Granted. Welcome back to DocuMind.')
      navigate('/')
    }
  }
      ...
      >
        <div className="bg-[#0f1117] rounded-[20px] p-10 md:p-12 border border-[rgba(255,255,255,0.07)] shadow-2xl relative overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between px-1">
                <label className="text-[10px] font-bold text-white/45 uppercase tracking-[0.2em]">Password</label>
                <Link to="#" className="text-[9px] font-bold text-[#4fa3f7] hover:text-[#7c5cfc] uppercase tracking-widest transition-colors">Forgot?</Link>
              </div>
              <div className="relative group">
            ...
            <button
              type="submit"
              disabled={loading}
              className="btn-primary w-full flex items-center justify-center space-x-3 py-4 text-[10px] tracking-[0.2em] transition-all"
            >
              {loading ? (
          ...
          <div className="mt-10 text-center text-[10px] font-bold text-white/45 uppercase tracking-widest">
            <p>
              New user?{' '}
              <Link to="/register" className="text-[#7c5cfc] hover:text-[#4fa3f7] transition-all">
                Sign Up
              </Link>
            </p>
          </div>
